...
public class CopyFileBuilder extends Builder implements SimpleBuildStep {
	...
	public CopyFileBuilder(String src, String des) {
		this.src = src;
		this.des = des;
	}
	...
	public void perform(Run<?, ?> run, FilePath workspace, Launcher launcher, TaskListener listener) {
		...
		ScriptMethod<?> method = new CopyFileMethod();
		JSONObject params = new JSONObject();
		params.put("src", src);
		params.put("des", des);
		params.put("cover", cover);
		logger.println("复制文件: src=" + src + ",des=" + des);
		try {
